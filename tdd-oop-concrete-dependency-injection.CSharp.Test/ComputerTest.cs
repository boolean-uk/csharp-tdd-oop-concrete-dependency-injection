...
namespace tdd_oop_concrete_dependency_injection.CSharp.Test
{
    class ComputerTest
    {
        ...
        public void shouldPlayGames()
        {
            PowerSupply myPsu = new PowerSupply();
            Computer myPc = new Computer(myPsu);
            Game gameDG = new Game("Duck Game");
            Game gameDDDA = new Game("Dragon's Dogma: Dark Arisen");
            Game gameM = new Game("Morrowind");

            myPc.installGame(gameDG);
            myPc.installGame(gameDDDA);

            Assert.That("Playing Duck Game", Is.EqualTo(myPc.playGame(gameDG)));
            Assert.That("Playing Dragon's Dogma: Dark Arisen", Is.EqualTo(myPc.playGame(gameDDDA)));
            Assert.That("Game not installed", Is.EqualTo(myPc.playGame(gameM)));
        }
